...
def process_doc_collection(collection_path, es_obj, need_raw, need_stemmed):
    for filename in os.listdir(collection_path):
        with open(os.path.join(collection_path, filename)) as file:
            ...
            print("Документ: " + str(file.name) + ". Кол-во: " + str(len(raw_documents)))
            counter = 0
            for raw_document in raw_documents:
                ...
                try:
                    ...
                    # print(doc_json)
                    print('Обработано ' + str(counter) + ' (' + doc_id + ')')
                except Exception:
                    print("Ошибка при обработке " + str(raw_document.find("docid").text))
                    continue
            file.close()
